...
if __name__ == "__main__":
    ...
    datasets = ['train', 'val', 'test2016', 'test2017', 'testcoco']
    model_name = args.model
    save_dir = os.path.join('image_feature', model_name)

    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    model = timm.create_model(model_name, pretrained=False, num_classes=0).to('cuda:0') # if use cpu, uncomment '.to('cuda:0')'
    model.eval()
    ...
    image_features = []
    for dataset in datasets:
        image_features.append(torch.load(os.path.join(save_dir, dic2[dataset]+'.pth')))
    res = torch.cat(image_features).detach().cpu().numpy()
    print('feature shape:', res.shape, ',save in:', save_dir+'/'+ model_name +'_image_features.npy')
    np.save(os.path.join(save_dir, model_name +'_image_features.npy'), res)
