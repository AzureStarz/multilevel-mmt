...
class SequenceGenerator(object):
    ...
    def _generate(
        ...
        **kwargs
        ...
        if 'image' in sample:
            encoder_input['image'] = sample['image']  #(batch,2048,7,7) or (batch,7,7,2048)

        # compute the encoder output for each beam
        encoder_outs = model.forward_encoder(encoder_input) #list
        multi_len = encoder_outs[0][0].size(0) # src_len+7 or src_len+49
        #print ("multi len", multi_len)

        src_tokens = encoder_input['src_tokens']
        img_len = multi_len - src_tokens.size(1)
        tmp_image = src_tokens.new(src_tokens.size(0), img_len).fill_(10)
        src_tokens = torch.cat((src_tokens, tmp_image),1)
...
class EnsembleModel(torch.nn.Module):
    ...
    def forward_encoder(self, encoder_input):
        if not self.has_encoder():
            return None
        if 'image' in encoder_input:
            img_embed = [model.img_encoder(encoder_input['image']) for model in self.models]
            return [model.encoder(encoder_input['src_tokens'], encoder_input['src_lengths'], encoder_input['phrase_tokens'], encoder_input['phrase_lengths'], img_features=img_embed[i]) for i,model in enumerate(self.models)]
        else:
