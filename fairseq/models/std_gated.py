...
@register_model('std_gated')
class StdGatedModel(FairseqEncoderDecoderModel):
    ...
    def forward(self, src_tokens, src_lengths, visions, phrase_tokens, phrase_lengths, prev_output_tokens, **kwargs):
        self.encoder.epoch = self.epoch
        encoder_out = self.encoder(src_tokens, src_lengths=src_lengths, visions=visions, phrase_tokens=phrase_tokens, phrase_lengths=phrase_lengths, **kwargs)
...
class TransformerEncoder(FairseqEncoder):
    ...
    def forward(self, src_tokens, src_lengths, visions, phrase_tokens, phrase_lengths, cls_input=None, return_all_hiddens=False, **unused):
        ...
        merge = torch.cat([text_repr, sentence_repr], dim=-1)
        gate = self.sigmoid(self.gate_dense(merge))
        # output = text_repr + gate * multilevel_repr
        output = text_repr + gate * sentence_repr
...
@register_model_architecture('std_gated', 'std_gated')
...
@register_model_architecture('std_gated', 'std_gated_iwslt_de_en')
...
@register_model_architecture('std_gated', 'std_gated_tiny')
...
@register_model_architecture('std_gated', 'std_gated_vatex')
